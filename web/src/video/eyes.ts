...
const LeftEyeIndex = [
  362, 398, 384, 385, 386, 387, 388, 466, 388, 374, 373, 372, 464, 463, 381, 380, 362
]
...
export function getIsBlinked(landmarkderResults: FaceLandmarkerResult): boolean {
  ...
  const averageRatio = ratioHistory.reduce((a, b) => a + b, 0) / ratioHistory.length;
...
function getFacePointsArea(landmarkderResults: FaceLandmarkerResult, index: number[]): number {
  const points = getPoints(landmarkderResults, index);
  return calculateArea(points);
}

function getPoints(
  landmarkderResults: FaceLandmarkerResult, index: number[]
): Point[] {
  const eyeArea: Point[] = index.map((index) => {
    return {
